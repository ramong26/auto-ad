...
4. 네이버 블로그 에디터에 붙여넣을 본문, 제목, 검토용 요약, 이미지 계획, 태그 3~10개와 출처 목록을 작성한다. 모바일에서 읽기 쉬운 짧은 문단과 소제목을 사용하고, 직접 실행 결과, 직접 만든 스크린샷, 재현 절차, 비교표, 선택 기준 중 둘 이상을 포함한다.
...
7. 네이버 블로그나 WordPress에 자동 게시하지 않고 `approved`, `publishing`, `published`로 바꾸지 않는다. `.env`와 인증정보를 읽거나 출력하지 않는다. `draft:finalize`가 Telegram 원고 완료 알림을 보낸다.
